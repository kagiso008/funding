...
class CalculateApsSPPage extends StatefulWidget {
  const CalculateApsSPPage({super.key});

  @override
  _CalculateApsSPPageState createState() => _CalculateApsSPPageState();
}

class _CalculateApsSPPageState extends State<CalculateApsSPPage> {
  ...
  Future<void> _fetchUserMarks() async {
    try {
      ...
      setState(() {
        ...
        aps = _CalculateApsSP(userMarks, mathType);
        isLoading = false;
      });
    } catch (error) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("YOUR APS AT SPU"),
