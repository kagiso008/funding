...
import 'package:funding/available_courses/tut_available.dart';

class CalculateTUTAPSPage extends StatefulWidget {
  const CalculateTUTAPSPage({super.key, required});

  @override
  _CalculateTUTAPSPageState createState() => _CalculateTUTAPSPageState();
}

class _CalculateTUTAPSPageState extends State<CalculateTUTAPSPage> {
  ...
  Future<void> _fetchUserMarks() async {
    try {
      ...
      setState(() {
        ...
        aps = _CalculateTUTAPS(userMarks);
        isLoading = false;
      });
    } catch (error) {
      setState(() {
  ...
  int _CalculateTUTAPS(Map<String, int?> marks) {
    int apsScore = 0;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("YOUR APS AT TUT"),
