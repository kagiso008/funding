...
class CalculateApsUFSPage extends StatefulWidget {
  const CalculateApsUFSPage({super.key, required});

  @override
  _CalculateApsUFSPageState createState() => _CalculateApsUFSPageState();
}

class _CalculateApsUFSPageState extends State<CalculateApsUFSPage> {
  ...
  Future<void> _fetchUserMarks() async {
    try {
      ...
      setState(() {
        ...
        aps = _CalculateApsUFS(userMarks);
        isLoading = false;
      });
    } catch (error) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("YOUR APS AT UFS"),
