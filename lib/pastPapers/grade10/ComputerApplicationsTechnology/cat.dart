...
class CATGrade10Page extends StatefulWidget {
  const CATGrade10Page({super.key});

  @override
  _CATGrade10PageState createState() => _CATGrade10PageState();
}

class _CATGrade10PageState extends State<CATGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/ComputerApplicationsTechnology');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/ComputerApplicationsTechnology/$fileName';
