...
class ConsumerStudiesGrade10Page extends StatefulWidget {
  const ConsumerStudiesGrade10Page({super.key});

  @override
  _ConsumerStudiesGrade10PageState createState() =>
      _ConsumerStudiesGrade10PageState();
}

class _ConsumerStudiesGrade10PageState
    extends State<ConsumerStudiesGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      ...
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/ConsumerStudies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      ...
      final String path = 'grade_10/ConsumerStudies/$fileName';
