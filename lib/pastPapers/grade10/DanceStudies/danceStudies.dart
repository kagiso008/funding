...
class DanceStudiesGrade10Page extends StatefulWidget {
  const DanceStudiesGrade10Page({super.key});

  @override
  _DanceStudiesGrade10PageState createState() =>
      _DanceStudiesGrade10PageState();
}

class _DanceStudiesGrade10PageState extends State<DanceStudiesGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      ...
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/DanceStudies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      ...
      final String path = 'grade_10/DanceStudies/$fileName';
