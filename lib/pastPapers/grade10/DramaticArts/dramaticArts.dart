...
class DramaticArtsGrade10Page extends StatefulWidget {
  const DramaticArtsGrade10Page({super.key});

  @override
  _DramaticArtsGrade10PageState createState() =>
      _DramaticArtsGrade10PageState();
}

class _DramaticArtsGrade10PageState extends State<DramaticArtsGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      ...
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/DramaticArts');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      ...
      final String path = 'grade_10/DramaticArts/$fileName';
