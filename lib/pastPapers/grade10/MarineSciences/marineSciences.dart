...
class MarineSciencesGrade10Page extends StatefulWidget {
  const MarineSciencesGrade10Page({super.key});

  @override
  _MarineSciencesGrade10PageState createState() =>
      _MarineSciencesGrade10PageState();
}

class _MarineSciencesGrade10PageState extends State<MarineSciencesGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      // Specify the path to the "MarineSciences" folder within "grade10"
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/MarineSciences');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      ...
      final String path = 'grade_10/MarineSciences/$fileName';
