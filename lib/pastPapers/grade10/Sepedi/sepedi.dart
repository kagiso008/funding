...
class SepediGrade10Page extends StatefulWidget {
  const SepediGrade10Page({super.key});

  @override
  _SepediGrade10PageState createState() => _SepediGrade10PageState();
}

class _SepediGrade10PageState extends State<SepediGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_10/Sepedi');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/Sepedi/$fileName';
