...
class SesothoGrade10Page extends StatefulWidget {
  const SesothoGrade10Page({super.key});

  @override
  _SesothoGrade10PageState createState() => _SesothoGrade10PageState();
}

class _SesothoGrade10PageState extends State<SesothoGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_10/Sesotho');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/Sesotho/$fileName';
