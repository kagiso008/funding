...
class SASLGrade10Page extends StatefulWidget {
  const SASLGrade10Page({super.key});

  @override
  _SASLGrade10PageState createState() => _SASLGrade10PageState();
}

class _SASLGrade10PageState extends State<SASLGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/SouthAfricanSignLanguage');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/SouthAfricanSignLanguage/$fileName';
