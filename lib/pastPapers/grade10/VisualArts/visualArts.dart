...
class VisualArtsGrade10Page extends StatefulWidget {
  const VisualArtsGrade10Page({super.key});

  @override
  _VisualArtsGrade10PageState createState() => _VisualArtsGrade10PageState();
}

class _VisualArtsGrade10PageState extends State<VisualArtsGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      // Specify the path to the "VisualArtsematics" folder within "grade10"
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_10/VisualArts');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      ...
      final String path = 'grade_10/VisualArts/$fileName';
