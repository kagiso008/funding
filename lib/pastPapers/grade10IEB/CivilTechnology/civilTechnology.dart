...
class CivilTechnologyGrade10Page extends StatefulWidget {
  const CivilTechnologyGrade10Page({super.key});

  @override
  _CivilTechnologyGrade10PageState createState() =>
      _CivilTechnologyGrade10PageState();
}

class _CivilTechnologyGrade10PageState
    extends State<CivilTechnologyGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/CivilTechnology');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/CivilTechnology/$fileName';
