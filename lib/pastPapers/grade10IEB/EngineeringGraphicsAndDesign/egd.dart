...
class EGDGrade10Page extends StatefulWidget {
  const EGDGrade10Page({super.key});

  @override
  _EGDGrade10PageState createState() => _EGDGrade10PageState();
}

class _EGDGrade10PageState extends State<EGDGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/EngineeringGraphicsAndDesign');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/EngineeringGraphicsAndDesign/$fileName';
