...
class HospitalityStudiesGrade10Page extends StatefulWidget {
  const HospitalityStudiesGrade10Page({super.key});

  @override
  _HospitalityStudiesGrade10PageState createState() =>
      _HospitalityStudiesGrade10PageState();
}

class _HospitalityStudiesGrade10PageState
    extends State<HospitalityStudiesGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      ...
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/HospitalityStudies');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      ...
      final String path = 'grade_10/HospitalityStudies/$fileName';
