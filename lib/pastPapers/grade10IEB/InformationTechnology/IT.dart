...
class ITGrade10Page extends StatefulWidget {
  const ITGrade10Page({super.key});

  @override
  _ITGrade10PageState createState() => _ITGrade10PageState();
}

class _ITGrade10PageState extends State<ITGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      // Specify the path to the "InformationTechnology" folder within "grade10"
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_10/InformationTechnology');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      ...
      final String path = 'grade_10/InformationTechnology/$fileName';
