...
class IsiXhosaGrade10Page extends StatefulWidget {
  const IsiXhosaGrade10Page({super.key});

  @override
  _IsiXhosaGrade10PageState createState() => _IsiXhosaGrade10PageState();
}

class _IsiXhosaGrade10PageState extends State<IsiXhosaGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_10/IsiXhosa');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/IsiXhosa/$fileName';
