...
class TshivendaGrade10Page extends StatefulWidget {
  const TshivendaGrade10Page({super.key});

  @override
  _TshivendaGrade10PageState createState() => _TshivendaGrade10PageState();
}

class _TshivendaGrade10PageState extends State<TshivendaGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_10/Tshivenda');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/Tshivenda/$fileName';
