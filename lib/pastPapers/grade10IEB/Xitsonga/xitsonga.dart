...
class XitsongaGrade10Page extends StatefulWidget {
  const XitsongaGrade10Page({super.key});

  @override
  _XitsongaGrade10PageState createState() => _XitsongaGrade10PageState();
}

class _XitsongaGrade10PageState extends State<XitsongaGrade10Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_10/Xitsonga');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_10/Xitsonga/$fileName';
