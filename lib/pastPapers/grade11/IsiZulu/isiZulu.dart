...
class IsiZuluGrade11Page extends StatefulWidget {
  const IsiZuluGrade11Page({super.key});

  @override
  _IsiZuluGrade11PageState createState() => _IsiZuluGrade11PageState();
}

class _IsiZuluGrade11PageState extends State<IsiZuluGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_11/IsiZulu');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/IsiZulu/$fileName';
