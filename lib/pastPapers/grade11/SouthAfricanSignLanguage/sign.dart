...
class SASLGrade11Page extends StatefulWidget {
  const SASLGrade11Page({super.key});

  @override
  _SASLGrade11PageState createState() => _SASLGrade11PageState();
}

class _SASLGrade11PageState extends State<SASLGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/SouthAfricanSignLanguage');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/SouthAfricanSignLanguage/$fileName';
