...
class MarineSciencesGrade11Page extends StatefulWidget {
  const MarineSciencesGrade11Page({super.key});

  @override
  _MarineSciencesGrade11PageState createState() =>
      _MarineSciencesGrade11PageState();
}

class _MarineSciencesGrade11PageState extends State<MarineSciencesGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      // Specify the path to the "MarineSciences" folder within "grade11"
      final List<FileObject> objects = await supabase.storage
          .from('pdfs')
          .list(path: 'grade_11/MarineSciences');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      ...
      final String path = 'grade_11/MarineSciences/$fileName';
