...
class SepediGrade11Page extends StatefulWidget {
  const SepediGrade11Page({super.key});

  @override
  _SepediGrade11PageState createState() => _SepediGrade11PageState();
}

class _SepediGrade11PageState extends State<SepediGrade11Page> {
  ...
  Future<void> _fetchPDFFiles() async {
    try {
      final List<FileObject> objects =
          await supabase.storage.from('pdfs').list(path: 'grade_11/Sepedi');
  ...
  Future<File?> _downloadPDF(String fileName) async {
    try {
      final String path = 'grade_11/Sepedi/$fileName';
